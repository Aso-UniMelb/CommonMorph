...
async def is_model_trained(langid: str):
  if os.path.exists(f'model/{langid}.model'):
    return {"message": f"Model is trained"}
  else:
...
# Prompt: how can I upload a file to the server using jquery in client and storing it on the server using FastAPI python?
...
def train(request: TrainRequest):
  ...
  if request.vocab_id:
    ...
    char_stoi, tags_stoi = ExtractVocabFromUniMorph(data, request.char_fields, request.tag_fields)
    # save vocabs to file
    with open(f'model/{request.langid}_vocab.json', 'w') as f:
      json.dump({"char_vocab": char_stoi, "tag_vocab": tags_stoi}, f)
    # encode dataset
    vocab = VocabFromJson(f'model/{request.langid}_vocab.json')
    
  dataset = encode_dataset(data, request.tag_fields, request.char_fields, vocab)
  # get the dataloaders
